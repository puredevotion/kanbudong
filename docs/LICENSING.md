...
- **Photographs of real menus/fascias/packages/labels.** The GDPR/PIPL
  concern is about *processing personal data* (a person's face in frame) and
  *trademark/protected-expression* in the photographed material, not about
...
- **Forvo audio / CSMSC-derived TTS checkpoints / MagicData.** Forvo's terms
  forbid *durable caching and redistribution* outright, which an offline PWA
  ...
  gated one.
- **Tatoeba sentences.** Ruled out primarily because the *register* is wrong
...
- **`kMandarin`/`kFrequency`/`kGradeLevel` fields from Unihan.** These were
  excluded for *data-quality* reasons (deprecated, small 1990s sources), not
...
- **Copyleft terms (LGPL, Arphic, ODbL's share-alike, CC BY-SA)** are
  triggered by *distribution*, and a Cloudflare Pages URL that friends open in
...
- **GDPR/PIPL and other personal-data law** track *processing personal data*,
